...
def format_item_for_display(item, player_character=None, show_price=False, is_sell_price=False, for_vendor=False):
    ...
    # Prepend the round-8 sprite icon (cryptic-shuffled for
    # potions/scrolls/spells, named map for everything else). Stays empty
    # if no sprite is mapped for this item type, so the row falls back to
    # plain text.
    try:
...
class Character:
    ...
    def add_status_effect(self, effect_name, duration, effect_type, magnitude=0, description="", resistance_element=None):
        ...
        if effect_type == 'confusion' and _has_confusion_immunity(self):
            ...
            return
        effect = StatusEffect(effect_name, duration, effect_type, magnitude, description, resistance_element=resistance_element)
