...
class WizardsCavernApp(toga.App):
    ...
    def build_layout_no_numpad(self, commands):
        ...
        right_margin = toga.Box(style=Pack(width=6, height=36))
        right_margin2 = toga.Box(style=Pack(width=6, height=36))
        right_margin3 = toga.Box(style=Pack(width=6, height=36))
        for btn in dpad_row1 + [self.create_spacer()] + cmd_row1 + [right_margin]:
            self.button_row_1.add(btn)
        for btn in dpad_row2 + [self.create_spacer()] + cmd_row2 + [right_margin2]:
            self.button_row_2.add(btn)
        for btn in dpad_row3 + [self.create_spacer()] + cmd_row3 + [right_margin3]:
            self.number_pad_box.add(btn)
    ...
    def process_command(self, cmd):
        ...
        if cmd == 'm' and gs.prompt_cntl == "game_loop":
            # Dwarf mining - break an adjacent ore vein wall
            ...
            if mines_used >= 3:
                ...
                return
            # Check if any adjacent ore vein exists
            ...
            if not has_vein:
                ...
                return
            add_log(f"{COLOR_CYAN}Which direction to mine? (n/s/e/w){COLOR_RESET}")
                ...
                if 0 <= ty < current_floor.rows and 0 <= tx < current_floor.cols:
                    ...
                    if room.room_type == current_floor.wall_char and room.properties.get('is_ore_vein'):
                        ...
                        if random.random() < 0.75:
                            # 70% ore ingredient, 30% sellable gem (gold)
 ...
 |               |
        ...
        if gs.prompt_cntl == "intro_story" or gs.prompt_cntl == "main_menu":
            ...
            if gs.prompt_cntl == "game_loop":
                ...
                # Add mine command for dwarves adjacent to detected ore veins
                if getattr(gs.player_character, 'race', '').lower() == 'dwarf':
                    mines_used = gs.dwarf_mines_per_floor.get(gs.player_character.z, 0)
                    if mines_used < 3:
                        has_adj_vein = False
                        for dy, dx in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                            ty, tx = gs.player_character.y + dy, gs.player_character.x + dx
